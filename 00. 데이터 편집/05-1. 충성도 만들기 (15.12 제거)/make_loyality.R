...
#warning
which(recent_day==min(make_loyality$recent_day))
##18761 , 19209 고객은 23개월동안 구매내역이 없음
recent_day[18761]<-0
recent_day[19209]<-0
...
for(i in 1:19383){
  price_mean[i]<-mean(raw_purchase[raw_purchase$고객번호==i,]$구매금액)
  pb$tick()
}
#warning
price_mean[18761]<-0
...
make_loyality[make_loyality$recent_day<0]<-0


###최근 경쟁사 이용일자에서 16100 마이너스 -> 경쟁사이용 0 인 고객이 있기에 범위값을 보존하는 선에서 빼줌
min(make_loyality[make_loyality$competitor_day>0,]$competitor_day)
max(make_loyality$competitor_day)

make_loyality$competitor_day<-make_loyality$competitor_day-16100
...
getwd()
write.csv(make_loyality,"make_loyality.csv")
str(make_loyality)
